...
int main(int argc, char** argv)
{
  ...
  for (int m = 0; m < NumMat; m++) {
    Real fa1 = 0.5*( 1.0 + 2.0*( Real(m)/Real(NumMat-1) ) );
    Real fa2 = 0.5*( 1.0 + 2.0*( Real(m)/Real(NumMat-1) ) * ( Real(m)/Real(NumMat-1) ) );
  ...
  myModel.writeOutputVTK("LV_FmM_", 0);
 ...
 // Loop through pressure
  ...
  /*  
  {
    ...
    for (int s = 1; s <= NumPsteps; s++) {
      ...
      myModel.writeOutputVTK("LV_FnM_", s);
      myModel.writeField("LV_FmM_Disp_", s);
	...
	Forces[i] = -F(BCid[i]);
      ...
      writeNeumannBC(BCid, Forces, "LV_FmM_Forces_", s);
    }
  }
...
*/
  ...
  // Next recompute material properties using EMFO
  {
	...
	DispFileNameStream << "LV_FmM_Disp_" << s << ".dat";
	ifstream DBCinp;
	...
	ForceFileNameStream << "LV_FmM_Forces_" << s << ".dat";
	ifstream NBCinp;
