...
int main(int argc,char** argv){
  ...
  bool flag_hessian_nonaxi=false;
  bool flag_hessian_TJHax=false;
  bool flag_hessian_TJHnonax=false;
  
  int vec_pos=0;
  
  while ( (opt=getopt(argc,argv,"tsckfohDSHpVMNTJ"))!=-1){
    switch (opt){
    case 'M':
    ...
    case 'H':
      ...
      break;
    case 'N':
      flag_hessian_nonaxi=true;
      break;
    case 'T':
      flag_hessian_TJHax=true;
      break;
    case 'J':
      flag_hessian_TJHnonax=true;
      break;
  ...
  if(flag_hessian==true){
	...
	c.Quad.gaussLegendreQuad_hess(Hessian,sol,hess_constr);
      else if(no_of_constr==2)
	c.Quad.gaussLegendreQuad_hess(Hessian_2constr,sol,hess_constr);
      else {
	cout<<"Wrong number of constraints"<<endl;
	exit(-1);
      }
	...
	c.Quad.gaussLegendreQuad_hess(Hessian_2constr,sol,mass_constr);
      else {
	cout<<"Wrong number of constraints"<<endl;
	exit(-1);
      }
      ...
      gsl_matrix_view constr = gsl_matrix_submatrix(hess_constr,u_len+phi_len,0,no_of_constr,u_len+phi_len);
      gsl_matrix* C = gsl_matrix_calloc(u_len+phi_len,no_of_constr);
      gsl_matrix_transpose_memcpy(C,&constr.matrix);
      //pprintMat(C);

      gsl_vector* tau = gsl_vector_calloc(no_of_constr);
      gsl_linalg_QR_decomp (C, tau);
      gsl_matrix* Q = gsl_matrix_calloc(u_len+phi_len,u_len+phi_len);
      gsl_matrix* R = gsl_matrix_calloc(u_len+phi_len,no_of_constr);
      gsl_linalg_QR_unpack (C, tau, Q,  R);
      gsl_matrix_view temp = gsl_matrix_submatrix(Q, 0, no_of_constr, u_len+phi_len, u_len+phi_len-no_of_constr);
      gsl_matrix* Q3 = gsl_matrix_calloc(u_len+phi_len,u_len+phi_len-no_of_constr);
      gsl_matrix_memcpy(Q3,&temp.matrix);
      //pprintMat(Q3);

      gsl_matrix* HQ3 = gsl_matrix_calloc(u_len+phi_len,u_len+phi_len-no_of_constr);
      gsl_matrix* Q3HQ3 = gsl_matrix_calloc(u_len+phi_len-no_of_constr,u_len+phi_len-no_of_constr);
      gsl_blas_dgemm(CblasNoTrans,CblasNoTrans,1,&hess.matrix,Q3,0,HQ3);
      gsl_blas_dgemm(CblasTrans,CblasNoTrans,1,Q3,HQ3,0,Q3HQ3);
      sol[Tot_len+7]=-1; // flag for mass computation (corresponds to beta)

      gsl_matrix* MQ3 = gsl_matrix_calloc(u_len+phi_len,u_len+phi_len-no_of_constr);
      gsl_matrix* Q3MQ3 = gsl_matrix_calloc(u_len+phi_len-no_of_constr,u_len+phi_len-no_of_constr);
      gsl_blas_dgemm(CblasNoTrans,CblasNoTrans,1,&Mass.matrix,Q3,0,MQ3);
      gsl_blas_dgemm(CblasTrans,CblasNoTrans,1,Q3,MQ3,0,Q3MQ3);

      //EigenValues using Arnoldi Iteration (Gives fast and accurate values)
      gsl_vector* eig=gsl_vector_calloc(Q3HQ3->size1);
      gsl_matrix* evec=gsl_matrix_calloc(Q3HQ3->size1,Q3HQ3->size2);
      arnoldi_eig(Q3HQ3,Q3MQ3, NO_OF_EIGS, eig, evec, 0);
  ...
  //------------- NONaxisymmetric hessian -----------------
  if(flag_hessian_nonaxi==true){
  ...
  //%%%%%%%%%%%%%%% Atha: Healey stability %%%%%%%%%%%%  
  if(flag_hessian_TJHax==true){
    ...
    if (constr==1){
      ...
      // terms suppressed in axisym should be included. The number of rows
      // are increased by a further 2+2(w,psi) to account for the constraints
      gsl_matrix* hess_constr = gsl_matrix_calloc(u_len+phi_len+4+3,u_len+phi_len+4);
      gsl_matrix* mass_constr = gsl_matrix_calloc(u_len+phi_len+4+3,u_len+phi_len+4);
	...
	printf("%le\n",eig->data[i]);
      ...
      cout << "------- iti : EVector --------"<<endl;
      //gsl_vector_fprintf(stdout,&constr_2.vector,"%e"); 
      cout<< "No of significant negative eigen values: " <<no_of_neg_eigs<<endl;
      printf("--------Inner product of eigen vectors with constraints-----------------\n");
      double overlap1, overlap2, overlap3;
      bool* flag_stable_ar = new bool[no_of_neg_eigs];
	...
	gsl_vector_view eigen_vector = gsl_matrix_row(evec, choice);
	//gsl_vector_fprintf(stdout,&eigen_vector.vector,"%f");
	gsl_blas_ddot(&constr_1.vector,&eigen_vector.vector,&overlap1);
	printf("<C1,ev> = %le\n",overlap1);
	gsl_blas_ddot(&constr_2.vector,&eigen_vector.vector,&overlap2);
	printf("<C2,ev> = %le\n",overlap2);
	gsl_blas_ddot(&constr_3.vector,&eigen_vector.vector,&overlap3);
	printf("<C3,ev> = %le\n",overlap3);
	if (fabs(overlap1) > 1e-6 || fabs(overlap2) > 1e-6 || fabs(overlap3)>1e-6){
	  flag_stable_ar[choice] = true;
	}
	else
	  flag_stable_ar[choice] = false;
      }
      bool flag_stable = flag_stable_ar[0];
      for(int i=1; i < no_of_neg_eigs; i++) flag_stable = flag_stable & flag_stable_ar[i];
      cout<<flag_stable<<endl;
					    
      delete[] flag_stable_ar;
      //pprintMat(evec);
...
//  %%%%%%%%%%%%% ITI healey stability %%%%%%%%%%%%%%%%%%%%%%
//  %%%%%%%%%%%%  Atha: healey stability nonaxi %%%%%%%%%%%%%%%%
  //------------- NONaxisymmetric hessian -----------------
  if(flag_hessian_TJHnonax==true){
