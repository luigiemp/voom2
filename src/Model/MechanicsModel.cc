...
namespace voom {
  ...
  void MechanicsModel::writeOutputVTK(const string OutputFile, int step)
  {
    ...
    for(int e = 0; e < NumEl; e++)
    {
      ...
      // Compute Green Lagrangian Strain for current element
      ...
      */
      // }
      out << Elist[0](0,0) << " " <<  Elist[0](0,1) << " " << Elist[0](0,2) << endl <<
      Elist[0](1,0) << " " <<  Elist[0](1,1) << " " << Elist[0](1,2) << endl <<
      Elist[0](2,0) << " " <<  Elist[0](2,1) << " " << Elist[0](2,2) << endl;
    }
    
    // Eff
    out << "SCALARS Eff double" << endl << "LOOKUP_TABLE default" << endl;
    ...
    for(int e = 0; e < NumEl; e++)
    {
      ...
      */
      // }
      vector<Vector3d> dirVecs = _materials[e]->getDirectionVectors();
      out << dirVecs[0].transpose() * Elist[0] * dirVecs[0] << endl;
    }
