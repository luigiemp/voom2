//-*-C++-*-
#ifndef __GaugeModel_h__
#define __GaugeModel_h__
...
namespace voom{
  ...
  // Model Results
  class GaugeModel: public EllipticModel {
  ...
  public:
     ...
     */
    GaugeModel(Mesh* aMesh, vector<GaugeLipid * > _materials, 
		   const uint NodeDoF);
  
    //! Input-file-based Constructor
    // GaugeModel(Mesh* myMesh, const string inputFile, const uint NodeDoF);
	...
	for (uint i = 0; i < numNodes; i++) 
	  for (uint j = 0; j < dim; j++){
	    double value1 = value*(double(rand())/double(RAND_MAX));
